...
namespace mpu {
	...
	void init(int mpu_addr) {
		...
		*/
		//_mpu->setAccOffsets(-14240.0, 18220.0, -17280.0, 15590.0, -20930.0, 12080.0);
		...
		*/
		//_mpu->setGyrOffsets(45.0, 145.0, -105.0);
		...
		*/
		_mpu->enableGyrDLPF();
		//_mpu->disableGyrDLPF(MPU6500_BW_WO_DLPF_8800); // bandwdith without DLPF
		...
		*/
		//_mpu->enableAccAxes(MPU6500_ENABLE_XYZ);
		//_mpu->enableGyrAxes(MPU6500_ENABLE_XYZ);
	}
	...
	float getResG() {
		return _mpu->getResultantG(getG());
	}
}
...
void loop() {
  xyzFloat gValue = _mpu->getGValues();
  xyzFloat gyr = _mpu->getGyrValues();
  float temp = _mpu->getTemperature();
  float resultantG = _mpu->getResultantG(gValue);
